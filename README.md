...
WindowSnapper is a small WPF utility for taking screenshots of one Windows application on a timer.
...
- Choose one notification trigger: off, every N screenshots, or every N minutes.
- Show a topmost, click-through toast and play a sound whenever that trigger fires.
- Use the bundled `notif.mp3` or choose a custom audio file, with a separate sound on/off switch.
- Adjust toast size and preview notifications from the main window.
- Remember settings in `%LOCALAPPDATA%\WindowSnapper\settings.json`.

## Notifications

Notification mode is mutually exclusive: choose **Off**, **Every N screenshots**, or **Timed reminder**. Active modes always show their toast when the condition is met. Notification audio can be muted separately without disabling the toast.

**Every N screenshots** counts successful captures and notifies on the chosen count. The counter starts over when a new capture session begins.

**Timed reminder** runs on its own interval while capture is active. Its timer is independent from the screenshot interval and screenshots do not reset it.

The default sound is `notif.mp3` in the application directory. Custom MP3, WAV, WMA, M4A/AAC, AIFF, and FLAC files can be selected from the UI. NAudio handles playback; formats that depend on Media Foundation require the matching Windows codec support.
...
Timed reminders show the elapsed capture-session runtime in `HH:MM:SS` format.
